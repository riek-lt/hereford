...
let userinputsub = ''; //Second user input for (silent) jumps. Not necessary
let slug = ''; //input for what schedule
let safety = 0; //Is a backup for the run number for if something goes wrong
let currentRun = 0; // Current run number state
let savedRun = ''; //String for current game name. Used for save feature
let finishSaveCheck = false; //??
let helpString =
  'You can use both ' +
  colors.green('green') +
  ' letter or words as commands;\n' +
  colors.green('"n"') +
  '  for ' +
  colors.green('next') +
  ' run\n' +
  colors.green('"p"') +
  '  for ' +
  colors.green('previous') +
  ' run\n' +
  colors.green('"sj"') +
  ' to ' +
  colors.green('silent jump') +
  ' to a run without changing text\n' +
  colors.green('"sn"') +
  ' to ' +
  colors.green('silent next') +
  ' to the next run \n' +
  colors.green('"j"') +
  '  to ' +
  colors.green('jump') +
  ...
  ' to ' +
  colors.green('reload') +
  ' the data from settings.txt\n' +
  colors.green('"nd"') +
  ' to go to the ' +
  colors.green('next deck') +
  ' of runs in your intermission';

// Start of program
slug = readline.question(
  'Please post the oengus slug OR full horaro URL for the marathon: '
);
data.call(slug); //Gets all data from the schedule
setTimeout(() => {
  //Gives time to do an API call
  initFiles();
  console.log(helpString); //Shows commands
  mainLogic();
  setInterval(() => {
    //This loops through the program
    mainLogic();
...
function mainLogic() {
  ...
  switch (userinput) {
    ...
    case 'sj':
    case 'silent jump':
      userinputsub = readline.question(
        'What run do you want to jump to silently (input a number)\nMax is ' +
          colors.green(data.scheduleLength) +
          ': '
      );
      try {
        ...
        console.log(
          'SJ successful. Current run is ' +
            colors.cyan(data.getRun(currentRun))
        );
      } catch (err) {
        currentRun = safety;
    ...
    case 'silent next':
      currentRun++;
      console.log(
        '"silent next" successful. Current run is ' +
          colors.cyan(data.getRun(currentRun))
      );
      break;
    case 'j':
    case 'jump':
      safety = currentRun;
      userinputsub = readline.question(
        'What run do you want to jump to (input a number)\nMax is ' +
          colors.green(data.scheduleLength) +
          ': '
      );
      try {
...
function writeToFiles(j, k) {
  //Writes to files. j= run number, k = unused
  if (typeof j == 'number') {
    //Is j a number variable?
    if (j > data.scheduleLength) {
      //Checks if input is bigger than amount of runs
      ...
      for (var i = 0; i < txtArray.length; i++) {
        if (data.runArray[i] != '') {
          //Primarily used to not display empty player 2/3/4, but are written
          console.log(
            colors.yellow(txtArray[i]) +
              colors.cyan(' -> ') +
              colors.green(data.runArray[i])
          );
        }
      ...
      if (settingsFile.twitchSync) {
        //Program to write game to the Twitch channel via API
...
function initFiles() {
  if (!fs.existsSync('./settings.txt')) {
    //If settings doesn't exist, make one
    settingsFile.create();
  }
  ...
  try {
    if (fs.existsSync(txtCategory)) {
      //Program checks if txtCategory exists, otherwise makes it. Not best way ofc
...
function savefileChecker() {
  ...
  for (var i = 0; i < data.scheduleLength; i++) {
    if (data.getRun(i) == savedRun) {
      //For every run in the marathon, see if a run with the same name is in
      finishSaveCheck = true;
      console.log(
        'Found run in this marathon (' +
          colors.red(data.getRun(i)) +
          ') that equals the previous last run. Do you want to resume where you left off? (' +
          colors.green('y') +
          '/' +
          colors.green('n') +
          ')'
